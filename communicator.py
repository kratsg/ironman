...
class Jarvis:
    ...
    def __call__(self, packet):
        ...
        """
        #print("communicator - packet: ", packet) # ECC
        if packet.request.header.type_id == 'CONTROL':
            #print("communicator - packet.response.transactions: ", packet.response.transactions) # ECC
    ...
    def __transaction__(self, transaction):
        ...
        if transaction.header.type_id == 'READ':
            ...
            print("Calling rmwbits. transaction.data", transaction.data)
            # need to return the register value prior to modifying it
            return(protocol.rmwbits(transaction.address, IPBusWords_long.build(transaction.data)))
        elif transaction.header.type_id == 'WRITE':
            print("Calling write. transaction.data", transaction.data)
